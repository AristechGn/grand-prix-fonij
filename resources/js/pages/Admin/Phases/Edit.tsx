...
import { EditPhaseProps } from '@/types';
...
export default function EditPhase({ edition, phase }: EditPhaseProps) {
  const { data, setData, put, processing, errors } = useForm<PhaseFormData>({
    name: phase.name || '',
    description: phase.description || '',
    start_date: phase.start_date || '',
    end_date: phase.end_date || '',
    location: phase.location || '',
    status: phase.status as 'pending' | 'active' | 'completed' || 'pending',
    order: phase.order || 1,
    color: phase.color || 'emerald',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    put(route('admin.editions.phases.update', [edition.id, phase.id]), {
      onSuccess: () => {
        window.location.href = route('admin.editions.phases.index', edition.id);
      }
    });
  };
  ...
  return (
    <AppLayout>
      <Head title={`Modifier la phase ${phase.name} - ${edition.name}`} />
              ...
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <CardTitle className="text-2xl flex items-center">
                    <PencilIcon className="h-6 w-6 mr-2 text-slate-500" />
                    Modifier la phase : {phase.name}
                  </CardTitle>
                  <CardDescription className="text-slate-500">
                    Modifiez les détails de cette phase de l'édition {edition.name}
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
