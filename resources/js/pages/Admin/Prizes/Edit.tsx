...
import { EditPrizeProps } from '@/types';

export default function EditPrize({ edition, prize }: EditPrizeProps) {
  const { data, setData, put, processing, errors } = useForm({
    name: prize?.name ?? '',
    description: prize?.description ?? '',
    amount: prize?.amount ?? 0,
    currency: (prize?.currency as 'GNF' | 'EUR' | 'USD') ?? 'GNF',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    put(route('admin.prizes.update', { prize: prize.id }));
  };

  return (
    <AppLayout>
      <Head title={`Modifier le prix - ${prize?.name}`} />
            ...
            <CardHeader className="bg-slate-50 border-b">
              <div className="flex items-center space-x-2">
                <div className="mr-2 flex">
                  <Link href={route('admin.editions.prizes.index', { edition: edition })}>
                ...
                <div>
                  <CardTitle className="text-2xl flex items-center">
                    ...
                    Modifier le prix: {prize.name}
                  </CardTitle>
                  <CardDescription className="text-slate-500">
                    Modifier les détails de ce prix pour l'édition {edition.name}
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
